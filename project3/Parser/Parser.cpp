...
namespace CodeAnalysis {
	...
	useParser::useParser(std::string file)
	{
		std::string fileSpec;
		...
		try
		{
			...
			{
				std::string temp = (*iter)->ToString();
				std::string temp1 = CodeAnalysis::stripQuotes(temp);
				std::string temp2 = FileSystem::Path::getName(temp1, true);
				statement_.push_back(temp2);
				iter++;
			}
			ClassAndFunction(pGlobalScope);
		}
	...
	const CodeAnalysis::useParser::statements & useParser::returnStatements() const
	{
		return statement_;
	}

	const CodeAnalysis::useParser::CFData & useParser::returnData() const
	{
		return CFdata_;
	}
}
